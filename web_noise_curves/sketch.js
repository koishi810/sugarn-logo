...
let sectorOverlap = 0.02;

let lastFrameTime = performance.now();
let baseTimeStep = 0.003;
let maxFrameStep = 2.5;

let logoRotation = Math.PI / 6;
...
let blackPixelScaleDeadZone = 0.006;
...
let cellCornerRoundnessMax = 2;
let cellCornerRoundnessSpeed = 0.045;
...
let cellEdgeSlackSpeed = 0.030;

let lineFrequencyRandomness = 0.55;
let lineAngleRandomness = 0.12;
let lineRadiusRandomness = 0.18;
let lineRandomnessSpeed = 0.010;
...
const defaultCurveFormula = "abs(cos(frequency * angle + time))";
...
class Debris {
  constructor() {
    this.respawn(random(-logoR * 1.2, logoR * 1.2));
    this.x += random(-logoR, logoR);
    this.y += random(-logoR, logoR);
  }

  respawn(startX) {
    const direction = random(TWO_PI);
    const speed = random(0.025, 0.12);
    this.x = startX;
    this.y = random(-logoR * 1.15, logoR * 1.15);
    this.vx = Math.cos(direction) * speed;
    this.vy = Math.sin(direction) * speed;
    if (Math.abs(this.vx) < 0.018) this.vx += this.vx < 0 ? -0.028 : 0.028;
    if (Math.abs(this.vy) < 0.010) this.vy += this.vy < 0 ? -0.016 : 0.016;
    this.rot = random(TWO_PI);
    this.vr = random(-0.0025, 0.0025);
  ...
  update(frameStep) {
    ...
    this.rot += this.vr * frameStep;
    const margin = logoR * 1.25 + this.size * 0.45;
...
function exportSettings() {
  downloadTextFile(`noise_curves_settings_${String(Math.floor(Date.now() / 1000))}.json`, currentSettingsText());
}
