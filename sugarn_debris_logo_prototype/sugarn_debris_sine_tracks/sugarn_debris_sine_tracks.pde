...
class Debris {
  ...
  void draw() {
    ...
    if (type == 0) drawSineTrack(1);
    if (type == 1) drawSineTrack(2);
    if (type == 2) drawSineTrack(3);
  ...
  void drawSineTrack(int bands) {
    float len = width * 1.8;
    for (int b = 0; b < bands; b++) {
      beginShape();
      for (int i = 0; i <= 48; i++) {
        float p = map(i, 0, 48, -len, len);
        float y = sin(p * 0.012 + t * 2.0 + b * 1.4 + rot) * size * 0.12 + (b - (bands - 1) * 0.5) * size * 0.18;
        vertex(p, y);
      }
      endShape();
    }
