...
class Debris {
  ...
  void draw() {
    ...
    if (type == 0) drawSuperellipse(1.4);
    if (type == 1) drawSuperellipse(2.6);
    if (type == 2) drawSuperellipse(4.0);
  ...
  void drawRing() {
    ...
    strokeWeight(weight);
    noFill();
    drawSuperellipse(4.2);
  }

  void drawSuperellipse(float n) {
    beginShape();
    for (int i = 0; i <= 48; i++) {
      float a = TWO_PI * i / 48.0;
      float ca = cos(a);
      float sa = sin(a);
      float x = sign(ca) * pow(abs(ca), 2.0 / n) * size * 0.42;
      float y = sign(sa) * pow(abs(sa), 2.0 / n) * size * 0.32;
      vertex(x, y);
    }
    endShape(CLOSE);
  }
