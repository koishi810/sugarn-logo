...
class Debris {
  ...
  void update() {
    ...
    // 连续包裹，不随机重生。这样不会因为形状/尺寸瞬间变化而跳帧。
    float margin = width * 1.2 + size * 2.0;
  ...
  void draw() {
    ...
    if (type == 0) drawRoseGrid(3);
    if (type == 1) drawRoseGrid(5);
    if (type == 2) drawRoseGrid(7);
  ...
  void drawRoseGrid(float k) {
    ...
    for (int rr = 0; rr < radialSteps; rr++) {
      ...
      for (int aa = 0; aa < angleSteps; aa++) {
        ...
        beginShape();
        roseVertex(a0, p0, k, phase);
        roseVertex(a1, p0, k, phase);
        roseVertex(a1, p1, k, phase);
        roseVertex(a0, p1, k, phase);
        endShape(CLOSE);
      }
    }
  }
