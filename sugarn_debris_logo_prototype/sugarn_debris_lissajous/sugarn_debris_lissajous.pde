...
class Debris {
  ...
  void draw() {
    ...
    if (type == 0) drawLissajous(2, 3);
    if (type == 1) drawLissajous(3, 5);
    if (type == 2) drawLissajous(4, 7);
  ...
  void drawLissajous(float ax, float ay) {
    beginShape();
    for (int i = 0; i <= 64; i++) {
      float u = TWO_PI * i / 64.0;
      float x = sin(ax * u + t * 1.6 + rot) * size * 0.44;
      float y = sin(ay * u + t * 1.1) * size * 0.34;
      vertex(x, y);
    }
    endShape();
  }
