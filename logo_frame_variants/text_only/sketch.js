...
function drawStraightLogoText() {
  ...
  const elapsed = (performance.now() - sketchStartTime) / 1000;
  const intro = smoothstep(textIntroDelay, textIntroDelay + Math.max(0.1, textIntroDuration), elapsed);
  drawStraightSugarTextIntro(intro, textYOffset, breathingLetterSpacing, breathingFontWeight, breathingNFontWeight);
}

function drawStraightSugarTextIntro(intro, y, letterSpacing, fontWeight, nWeight) {
  ctx.save();
  ...
  const startS = { x: -sWidth / 2, y, angle: startAngle };
  const startN = { x: sWidth / 2 + nGap + nWidth / 2, y, angle: startAngle };
  const insertStart = { x: 0, y, angle: startAngle };
  const sugarLayout = straightTextLayout("sugar", y, letterSpacing, nGap, nWidth).sugar;
  const finalN = straightTextLayout("sugar", y, letterSpacing, nGap, nWidth).n;
  ...
  drawArcLetter("n", superscriptPoint(nAnchor), 1);
  ctx.restore();
}

function straightTextLayout(text, y, letterSpacing, gap, nextWidth = 0) {
  setupLogoTextStyle(textFontSize, textFontWeight, textGray);
  const chars = Array.from(text);
  const widths = chars.map((char) => ctx.measureText(char).width);
  const spacing = letterSpacing || 0;
  const sugarWidth = widths.reduce((sum, width) => sum + width, 0) + spacing * Math.max(0, chars.length - 1);
  const totalWidth = sugarWidth + gap + nextWidth;
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    cursor += widths[i] + (i < chars.length - 1 ? spacing : 0);
  }

  return {
    sugar,
    n: {
      x: -totalWidth / 2 + sugarWidth + gap + nextWidth / 2,
      y,
      angle: Math.PI / 2,
    },
  };
